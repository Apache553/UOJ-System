<?php
	function handleRegisterPost() {
		...
		if (!DB::selectCount("SELECT COUNT(*) FROM user_info"))
			DB::query("insert into user_info (username, email, password, svn_password, register_time, usergroup, nickname) values ('$username', '$esc_email', '$password', '$svn_pw', now(), 'S', '$username')");
		else
			DB::query("insert into user_info (username, email, password, svn_password, register_time, nickname) values ('$username', '$esc_email', '$password', '$svn_pw', now(), '$username')");
